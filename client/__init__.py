...
__all__ = ['__version__', 'MessageClient', 'ConcurrentMessageClient']
...
class MessageClient(object):
    ...
    @_timeout
    def message_handle(self, contents_list, para_dict, blocking=True, is_tokenized=False):
        """
            with MessageClient() as mc:
                # handle message
                mc.message_handle(['First do it',
                                  'then do it right',
                                  'then do it better'])

        :param data_list: data_list[0] must data list, such as [str1, str2, str3]
                          data_list[1:] is the other information parameter
        :param blocking:
        :param is_tokenized:
        :return:
        ...
        """
        if is_tokenized:
            self._check_input_lst_lst_str(contents_list)
        else:
            self._check_input_lst_str(contents_list)

        if self.length_limit and not self._check_length(contents_list, self.length_limit, is_tokenized):
        ...
        contents = _unicode(contents_list)
        data_list = []
        data_list.append(contents)
        data_list.append(para_dict)
        self._send(jsonapi.dumps(data_list), len(contents))
    ...
    def message_handle_async(self, batch_generator, max_num_batch=None, delay=0.1, is_tokenized=False):
        ...
        def run():
            ...
            for texts in batch_generator:
                self.message_handle(texts, blocking=False, is_tokenized=is_tokenized)
                cnt += 1
    ...
    def _check_input_lst_lst_str(texts):
        ...
        for s in texts:
            MessageClient._check_input_lst_str(s)
...
class ConcurrentMessageClient(MessageClient):
    def __init__(self, max_concurrency=10, **kwargs):
        ...
        self.available_mc = [MessageClient(**kwargs) for _ in range(max_concurrency)]
        self.max_concurrency = max_concurrency

    def close(self):
        for mc in self.available_mc:
            mc.close()

    def _concurrent(func):
        def arg_wrapper(self, *args, **kwargs):
            try:
                mc = self.available_mc.pop()
                f = getattr(mc, func.__name__)
                r = f if isinstance(f, dict) else f(*args, **kwargs)
                self.available_mc.append(mc)
                return r
            except IndexError:
    ...
    def message_handle_async(self, **kwargs):
        raise NotImplementedError('Async encoding of "ConcurrentBertClient" is not implemented yet')
