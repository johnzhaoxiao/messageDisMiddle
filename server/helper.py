...
def set_logger(context, verbose=False,logfile=None):
...
def send_ndarray(src, dest, X, req_id=b'', flags=0, copy=True, track=False):
    ...
    if type(X) == list and type(X[0]) == dict: # 分类for sink发送消息的处理
        md = dict(dtype='json', shape=(len(X[0]['infer_rst']), 1))
    elif type(X) == dict: # 分类 bertwork 发送消息的处理
        md = dict(dtype='json', shape=(len(X['infer_rst']), 1))
    else:
        md = dict(dtype='str', shape= np.array(X).shape)
    print('send a numpy array with metadata: ', md)
...
def get_args_parser():
    ...
    group1 = parser.add_argument_group('File Paths',
                                       'config the path, vocab file path、pb models path.')
    group1.add_argument('-zeromq_sock_tmp_dir', type=str, default=None,
                        help='tmp file for zeromq sockets')
    # group1.add_argument('-word2id_path', type=str, default='./model_proxy_file/word2id.pkl',
    #                     help='word2id path')
    group1.add_argument('-run_script_dir',type=str, default='./')
    group1.add_argument('-logger',type=str,default='./logger.txt')


    # group2 = parser.add_argument_group('model Parameters',
    #                                    'config how model predict work')
    # group2.add_argument('-max_seq_len', type=int, default=128,
    #                     help='maximum length of a sequence')
    # group2.add_argument('-model_batch_size', type=int, default=64,
    #                     help='model inference batch_size')
...
def check_tf_version():
    ...
    assert int(tf_ver[0]) >= 1 and int(tf_ver[1]) >= 7, 'Tensorflow >=1.10 is required!'
    return tf_ver
...
def auto_bind(socket):
    ...
    if os.name == 'nt':  # for Windows
        ...
        try:
            #os.environ['ZEROMQ_SOCK_TMP_DIR'] = '/workspace/devspace/messageDisMiddle/messageDisMiddle/'
...
def get_shutdown_parser():
    parser = argparse.ArgumentParser()
    parser.description = 'Shutting down a Server instance running on a specific port'

    parser.add_argument('-ip', type=str, default='localhost',
                        help='the ip address that a Server is running on')
    parser.add_argument('-port', '-port_in', '-port_data', type=int, required=True,
                        help='the port that a Server is running on')
